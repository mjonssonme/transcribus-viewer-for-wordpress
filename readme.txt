...
This plugin provides a full-featured system for importing Transkribus documents into WordPress and embedding them anywhere via a custom "Transcribus Document" Gutenberg block - just upload the export ZIP through the async background uploader, then drop the block into any post or page. The viewer displays the document's description followed by the interactive image and text panes.
